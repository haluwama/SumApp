var GameRoot = function (profil) {
	this.name = "GameRoot";
	this.profile = profil;
	this.number = this.randomNumber(2, 98);
...
GameRoot.prototype = {
	...
	randomNumbers: function () {
		var arr = [];
		var liczba1 = this.randomNumber(0, 100);
		arr.push(liczba1);
		var i = 1;
		while (i < 4) {
			var liczba2 = this.randomNumber(0, 100);
			if (liczba1 !== liczba2 && liczba2 !== this.number) {
				arr.push(liczba2);
				liczba1 = liczba2;
				i++;
			}
	...
	check: function () {
		if (this.number === this.choice.number)
			return true;
		else
			return false;
	}
